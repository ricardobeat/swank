...
describe('Swank', function (){
  ...
  describe('ngrok', function(){
    ...
    before(function(done){
      run('bin/swank', ['--ngrok', '--watch', 'test/fixtures/'], null, function (data, child_done){
        stop_proc = child_done;
    ...
    it('should allow ngrok tunnelling', function (done){
      open(url, function (res){
        expect(res.statusCode).to.equal(200);
        expect(res.body).to.contain('Hello, World');
    ...
    it('should allow livereload through ngrok', function(done){
      open(url, function (res){
    ...
    it('should be tunneling livereload through ngrok', function(done){
